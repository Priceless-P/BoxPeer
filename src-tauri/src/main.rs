...
use tauri::{Manager, State, async_runtime::spawn};
mod net;
mod node;
...
use crate::node::{load_peer_info, save_peer_info};

// Define the state structure to manage the P2PCDNClient instance.
struct AppState {
    client: Arc<AsyncMutex<P2PCDNClient>>,
    network_events: Arc<AsyncMutex<Pin<Box<dyn futures::Stream<Item = net::Event> + Send>>>>,
}

// Tauri command to start listening on a given address.
#[tauri::command]
...
async fn request_file(
    ...
    let cid = cid.parse().map_err(|e| format!("Request file error: {}", e))?;
    let path = PathBuf::from(save_path);
    let mut client = state.client.lock().await;
    client
        .request_file(cid, path)
        .await
        .map_err(|e| e.to_string())
}
...
// mod net;
//
// use crate::content::{cache_chunk_locally, ContentManager};
// use crate::node::{
...
//         let content_chunk_index = format!("{}_chunk_{}", actual_name, chunk_index);
